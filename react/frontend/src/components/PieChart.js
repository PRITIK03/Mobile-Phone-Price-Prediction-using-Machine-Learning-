...
const PieChart = () => {
  // Sample static data
  const data = [
    { label: 'X', value: 40, color: '#4e79a7' },
    { label: 'Y', value: 30, color: '#f28e2b' },
    { label: 'Z', value: 30, color: '#e15759' },
  ];
    ...
    <div style={{ width: '180px', margin: '20px auto' }}>
      ...
      <svg width="150" height="150" viewBox="0 0 32 32">
        {data.map((d, i) => {
          ...
          const x1 = 16 + 16 * Math.sin(startAngle);
          const y1 = 16 - 16 * Math.cos(startAngle);
          const x2 = 16 + 16 * Math.sin(endAngle);
          const y2 = 16 - 16 * Math.cos(endAngle);
